...
get_data<- function(r){
  ...
  data$`NG: SUN` = replace(data$`NG: SUN` , which(data$`CO2 Emissions Generated`  <= 0), 0)
...
for (r in countries) {
  data<- get_data(r)
  load(paste0("dlnm_mod_June30_f_", r, ".Rda"))
...
#1:   MIDW      100    -1434.4 -390.8546 -6710.356 -14.344
...
#   region increase cumulative     lower     upper        V2
#1:   MIDA     1850   -4301.64 -2089.143 -10065.67 -2.325211
...
for (r in countries) {
  ...
  s <- quantile(data$`NG: SUN`, 
                probs = q, na.rm = T)
  load(paste("dlnm_mod_June30_", r, ".Rda", sep=""))
...
ggplot(plot_dat_all,
  ...
  scale_x_continuous(expand = c(0, 0)) +
  #scale_y_continuous(expand = c(0, 0)) +
  scale_y_continuous(expand = c(0, 0),limits = c(-1800,0)) + #ylimit is set 
...
p1 <- ggplot(data2, aes(x=month_year)) +
  ...
  scale_x_date(limit=c(as.Date("2018-07-01"),as.Date("2023-07-01")))
