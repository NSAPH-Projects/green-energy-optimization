Hourly solar/coal data Daniel Mork 3/18/2023
...
complete <- which(complete.cases(lag_dat) & 
                          complete.cases(data[, .(NG..COL, demand)]))

# Distributed lag models
# control for demand

# Linear lagged model
lin_mod <- tdlnm(NG..COL ~ ns(index, df = 24) * ns(Hour, df = 4),           
                  data[complete],
...
plot(s_lin_mod, start.time = -lags,
     xlab = "Hours lead (-) / lag (+)", 
     ylab = "Change in coal generation per unit increase in solar",
     main = "Hourly data")
```

```{r}
# Nonlinear lagged model
dlnm_mod <- tdlnm(I(-1*NG..COL) ~ ns(index, df = 24) * ns(Hour, df = 4),           
                  data[complete],
                  exposure.data = lag_dat[complete,],
...
```{r}
s_dlnm_mod <- summary(dlnm_mod, cenval = mean(data$NG..SUN), conf.level = .9)
...
{p1 <- plot(s_dlnm_mod, "slice", time = -1, start.time = -lags,
            xlab = "Solar generation", ylab = "Decrease in coal generation",
            main = "1 hours lead")
...
{p1 <- plot(s_dlnm_mod, "slice", val = 2000, start.time = -lags,
            xlab = "Hours lead (-) / lag (+)", ylab = "Decrease in coal generation",
            main = "Solar = 2000")
