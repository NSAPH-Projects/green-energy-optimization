...
for (i in (1:length(countries))){
  ...
  data_list[[i]]<-data #Note: this data contain NA values
...
for (i in (1:length(countries))){
  ...
  for (s in 1:5)
    s0[s, s] <- 0.819^2
  #dlnm_mod_list[[i]]
  dm <- combine.models(lapply(1:10, function(j) {
                                tdlnm(I(-1*CO2.Emissions.Generated) ~ ns(index, df = 24) + ns(index, df = 4) * ns(Hour, df=5) + demand,  #factor(hour)<-  
                            #ns(index, df = 24) + ns(index, df = 4) * factor(Hour)
                                              ...
                                              monotone = T,
                                              time.split.prob = rep(c(10, 3), c(5, 8)),
                                              monotone.gamma0 = g0,
                                              monotone.sigma = s0#,
                                              #n.burn=100, n.iter=100
                                              )
  }))
  #dm<-dlnm_mod_list[[i]]
  save(dm,file=paste("dlnm_mod_June12_",countries[i],".Rda",sep=""))
...
#linear model
library(mgcv)
m_linear <- lm(CO2~ SUN + ..., data) #or exposure.se = 10000
m_smooth <- gam(formula + s(SUN), data)
AIC(m_smooth)
AIC(m_linear)
```
...
for (i in (1:length(countries))){
  ...
  jpeg(paste("qqplot_June12_",countries[i],".jpg", sep=""))
  qqnorm(scale(dm$Y - dm$Yhat)); abline(0, 1)
  dev.off()
}
...
for (i in (1:length(countries))){
  ...
  jpeg(paste("difference_June12_",countries[i],".jpg", sep=""))
  plot(dm$Y, dm$Yhat-dm$Y)
  dev.off()
}
```
...
year_wise_analysis <- function(country, start_date, end_date){
  ...
  # Create lagged data
  lags <- 24
  lag_dat <- as.matrix(data[, shift(NG..SUN, 0:24, type = "lag")])
  #lead_dat <- as.matrix(data[, shift(NG..SUN, 10:1, type = "lead")])
  #lag_dat <- cbind(lead_dat, lag_dat)
  # data[, demand_ma10 := rollmean(demand, k = 10)] -- CHECK! 
  # Complete data rows
...
run_analysis <- function(combo, base, other){
    ...
    save(dlnm_mod_directed,file=paste("dlnm_mod_directed_June12_",paste(base, other, sep="_"),".Rda", sep=""))
...
countries <- c('CAL', 'CAR', 'CENT', 'FlA', 'MIDA', 'MIDW', 'NE', 'NW', 'SE', 'SW', 'TEN', 'TEX')
base_countries =c(5,5,6,6,8,8,9,9,9,9,10,11,11,11,12)#c(1,1,2,2,3,3,3,3,5,5,5,6,6,8,8,9,9,9,9,10,11,11,11,12)
other_countries =c(6,11,9,11,1,10,2,4,6,11,1,2,5,9,3)#c(10,8,5,9,6,8,10,12,2,6,11,9,11,1,10,2,4,6,11,1,2,5,9,3)
