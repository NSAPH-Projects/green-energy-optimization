...
for (i in (1:length(countries))){
  ...
  #dm<-dlnm_mod_list[[i]]
  save(dm,file=paste("dlnm_mod_June9_",countries[i],".Rda",sep=""))
...
for (i in (1:length(countries))){
  ...
  jpeg(paste("qqplot_June9_",countries[i],".jpg", sep=""))
...
for (i in (1:length(countries))){
  ...
  jpeg(paste("difference_June9_",countries[i],".jpg", sep=""))
...
for (i in (1:length(countries))){
  ...
  jpeg(paste("lineplot_May30_",countries[i],".jpg", sep=""))
...
for (i in (1:length(countries))){
  ...
  if(i>9){
    load(paste("dlnm_mod_May30_",countries[i],".Rda",sep=""))
    s_dlnm_mod <- summary(dm, cenval = 0, conf.level = .9)
    jpeg(paste("heatmap_May30_",countries[i],".jpg", sep=""))
...
run_analysis <- function(combo, base, other){
    ...
    # Create lagged data
    lags <- 6
    lag_dat <- as.matrix(data[, shift(NG..SUN, 0:6, type = "lag")])
    #lead_dat <- as.matrix(data[, shift(NG..SUN, 10:1, type = "lead")])
    #lag_dat <- cbind(lead_dat, lag_dat)
    complete<- which(complete.cases(lag_dat) & 
                                complete.cases(data[, .(CO2.Emissions.Generated, demand)]))
    ...
    dlnm_mod_directed <- combine.models(lapply(1:5, function(j) {
                                    tdlnm(I(-1*CO2.Emissions.Generated) ~ ns(index, df = 24) + ns(index, df = 4)* ns(Hour, df = 5) + demand,       
                                                  data[complete],
                                                  exposure.data = lag_dat[complete,],
                                                  exposure.splits = seq(quantile(lag_dat[complete,], .05), quantile(lag_dat[complete,], .95), length.out = 10), #5%and 95% percentile
                                                  #n.burn = 500, n.iter = 1000,
                                                  monotone = T,
                                                  time.split.prob = rep(c(10, 1), c(4, 2)))
      }))
    
    save(dlnm_mod_directed,file=paste("dlnm_mod_directed_May30_",paste(base, other, sep="_"),".Rda", sep=""))
