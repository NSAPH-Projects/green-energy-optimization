...
for (i in (7:length(countries))){
  ...
  dlnm_mod_list[[i]] <- combine.models(lapply(1:5, function(j) {
                                tdlnm(I(-1*CO2.Emissions.Generated) ~ ns(index, df = 24) * ns(Hour, df = 4) + demand,       
                                              ...
                                              exposure.splits = seq(quantile(lag_dat[complete,], .05), quantile(lag_dat[complete,], .95), length.out = 12), #5%and 95% percentile
...
```{r}
y_upper_list<- c(12000, 6000, 500, 20000, 40000, 1500)
for (i in (1:length(countries))){
  dlnm_mod <- dlnm_mod_list[[i]]
  ...
  p5 <- plot(s_dlnm_mod, "slice", time = 5, start.time = 0,
             xlab = "", ylab = "", 
             main = "5 hours lag") + scale_y_continuous(limits=c(-200,y_upper_list[i]))
  p6 <- plot(s_dlnm_mod, "slice", time = 8, start.time = 0,
             xlab = "", ylab = "", 
             main = "8 hours lag") + scale_y_continuous(limits=c(-200,y_upper_list[i]))
  p7 <- plot(s_dlnm_mod, "slice", time = 12, start.time = 0,
             xlab = "", ylab = "", 
             main = "12 hours lag") + scale_y_continuous(limits=c(-200,y_upper_list[i]))
...
run_analysis <- function(combo, base, other){
    ...
    save(dlnm_mod_directed,file=paste("dlnm_mod_directed_",paste(base, other, collapse="_"),".Rda", sep=""))
...
```{r}
generate_plots<- function(base, other){
  load(paste("dlnm_mod_directed_","dlnm_mod_directed_",paste(base, other, collapse="_"),".Rda", sep=""))
    s_dlnm_mod <- summary(dlnm_mod_directed, cenval = 0, conf.level = .9)
    
    #lineplot 
    jpeg(paste("lineplot_directed_","dlnm_mod_directed_",paste(base, other, collapse="_"),".jpg", sep=""))
    plot(0:lags, s_dlnm_mod$splitProb, type = 'l', ylim = 0:1,
         main = paste(countries, collapse="_"),
         xlab = "lag (+)", ylab = "Prob effect"); abline(h = 0.95, lty = 2)
    dev.off()
    ...
    #heatmap
    jpeg(paste("heatmap_directed_","dlnm_mod_directed_",paste(base, other, collapse="_"),".jpg", sep=""))
    plot(s_dlnm_mod, start.time = 0,
       xlab = "Lag hour",
       ylab = "Solar generation",
       flab = "Decreased CO2",
    ...
    #lagged plots
    jpeg(paste("continuous_directed_","dlnm_mod_directed_",paste(base, other, collapse="_"),".jpg", sep=""))
    p2 <- plot(s_dlnm_mod, "slice", time = 0, start.time = 0,
               xlab = "Solar generation", ylab = "Decrease in CO2 generation",
               main = "Same time") + scale_y_continuous(limits=c(-200,5000))
    p3 <- plot(s_dlnm_mod, "slice", time = 1, start.time = 0,
