...
lags=12
models_filename = "tdlm_models_lag12"

solar_medians <- solar_cap %>%
  filter(year == 2022) %>%
  group_by(Region) %>%
  summarise(total_median = sum(solar_median_mwh, na.rm = TRUE), .groups = 'drop')

solar_medians <- solar_medians %>%
  mutate(
    "0.05" = total_median * 0.05,
    "0.1" = total_median * 0.10,
    "15" = total_median * 0.15,
    "20" = total_median * 0.20
  )

# Print the array
print("coal")
solar_medians[,-1]*1.01
...
p2a<- ggplot(co2_offset[Percent == x & Hour %in% c(12, 20)]) +
  ...
  png(paste("output/", r, "_hourly_together_median.png", sep=""), width=800, height=400)
  print(p2a)
  dev.off() 
```
...
png(paste("output/", r, "_cumulative_median.png", sep=""), width=800, height=400)
print(p2b)
dev.off()
```
...
png(paste("output/cumulative_combined_median.png", sep=""), width=1000, height=1000)
...
png(paste("output/daily_median.png", sep=""), width=800, height=600)
print(p_daily)
dev.off()
```
```{r}
...
png(paste("output/total_median.png", sep=""), width=1000, height=500)
print(p_total)
dev.off()

#Print total annual CO2 change by Percent
...
png(paste("output/interregional_median.png", sep=""), width=1000, height=1000)
...
all_results[r1 == r2, sum(as.numeric(effect_5), na.rm = T) * 365 / 1000000, by = r1][,sum(V1)]
all_results[r1 == r2, sum(as.numeric(effect_10), na.rm = T) * 365 / 1000000, by = r1][,sum(V1)]
all_results[r1 == r2, sum(as.numeric(effect_15), na.rm = T) * 365 / 1000000, by = r1][,sum(V1)]
all_results[r1 == r2, sum(as.numeric(effect_20), na.rm = T) * 365 / 1000000, by = r1][,sum(V1)]
```
